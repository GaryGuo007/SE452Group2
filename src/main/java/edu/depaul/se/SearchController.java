	 ...
	 @RequestMapping(value="/performSearch")
		public ModelAndView performSearch(@ModelAttribute("search")Search search, ModelMap model) {
			...
			for(Worker x : cust){
				...
				//Form validation will prevent city, state, cost, and skill from being null. 
				//The only check will need to be for zip in which isn't required in the cast a large city has multiple zipcodes. 
				...
				if(cost <= tempCost && tempSkill.equals(skill.toLowerCase()) && tempCity.equals(city.toLowerCase()) && tempState.equals(state.toLowerCase())){
					if(zipcode.length()>0){
